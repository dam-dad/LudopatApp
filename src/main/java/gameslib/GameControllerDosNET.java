...
public class GameControllerDosNET implements Initializable {
	...
	/**
	 * Activamos las funcionalidades para este jugador. Normalmente llamado cuando
	...
	private void onChangedPlayer(Player ov, Player nv) {
		...
		if (ov != null) {
			...
			Optional<Player> ovPlayer = dosGame.getCurrentPlayers().stream()
					.filter(p -> p.getPlayerInfo().getUserID() == ov.getPlayerInfo().getUserID()).findFirst();
		...
		if (nv != null) {
			...
			Optional<Player> nvPlayer = dosGame.getCurrentPlayers().stream()
					.filter(p -> p.getPlayerInfo().getUserID() == nv.getPlayerInfo().getUserID()).findFirst();
	 ...
	 */
	// Joel: Usenlo en el endgame o el ultimo que tire carta se va a quedar con 1 en
	// el label potats
	...
	/**
	 * Notifica a los jugadores de la desconexion de otro de los jugadores y vuelve
	 * al menu
	 */
